...
function isSumOfCubes(s) {
    // ...
    let findValidNumbers = [];
    let currentNum = '';
  
    for (let i = 0; i < s.length; i++) {
      if (currentNum.length === 3 || currentNum.includes('-')) {
        findValidNumbers.push(currentNum);
        currentNum = '';
      }
  
      if (
        (!isNaN(s.charAt(i)) && s.charAt(i) !== ' ') ||
        (s.charAt(i) === '-' && !isNaN(s.charAt(i)))
      ) {
      ...
      if (i + 1 === s.length && currentNum.length > 0) {
        findValidNumbers.push(currentNum);
      }
    }
  
    const cubedNums = findValidNumbers.map((num) => {
      return num
        .split('')
        .map((n) => parseInt(n) ** 3)
    ...
    let sum = 0;
    let result = '';
    cubedNums.forEach((num, i) => {
      if (num == findValidNumbers[i]) {
        result += `${num} `;
        sum += num;
      }
    });
  
    return result === '' ? 'Unlucky' : result + `${sum} ` + 'Lucky';
  }