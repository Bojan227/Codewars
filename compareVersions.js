...
console.log(version1, version2)
    if(firstVersion.length === 1 && secondVersion.length === 1) return parseFloat(firstVersion) >= parseFloat(secondVersion);

    for(let i = 0; i < firstVersion.length; i++){
        
        if(secondVersion[i] === undefined) return true  
       if(firstVersion[i] === undefined) return false 
       console.log()
        if(firstVersion[i] !== secondVersion[i]){
            if(parseFloat(firstVersion[i]) >= parseFloat(secondVersion[i])){
                return true
            }else return false
        }
