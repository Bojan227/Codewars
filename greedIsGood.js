...
function score( dice ) {
    ...
    for(let key in cache){
      ...
      if(key === '1'){
        if(cache[key] >= 3){
          score += (Math.floor(parseInt(cache[key]) / 3) * 1000) + (parseInt(cache[key] % 3) * 100)
          
        }else {
          score += parseInt(cache[key])  * 100
          
        }
      }else if(key === '5'){
        if(cache[key] >= 3){
          score += (Math.floor(parseInt(cache[key]) / 3) * 500) + (parseInt(cache[key] % 3) * 50)
          
        }else {
          score += parseInt(cache[key])  * 50
          
        }
  
      }else if(cache[key] >= 3){
        score += parseInt(`${key}00`)
