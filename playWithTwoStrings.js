...
function countOcc(obj, a) {
  ...
  function workOnStrings(a, b) {
    //..
    const occurancesA = countOcc({}, a.toLowerCase());
    const occurancesB = countOcc({}, b.toLowerCase());
  
    console.log(occurancesA, occurancesB);
  
    let resA = '';
    let resB = '';
    for (let i = 0; i < b.length; i++) {
      const occToLowerCase = occurancesA[b[i].toLowerCase()];
  
      if (occToLowerCase) {
        if (b[i].charCodeAt() >= 97 && occToLowerCase % 2 === 0) {
          resA += b[i].toLowerCase();
        } else if (b[i].charCodeAt() <= 90 && occToLowerCase % 2 === 0) {
          resA += b[i].toUpperCase();
        } else if (b[i].charCodeAt() >= 97 && occToLowerCase % 2 !== 0) {
          resA += b[i].toUpperCase();
        } else if (b[i].charCodeAt() <= 90 && occToLowerCase % 2 !== 0) {
          resA += b[i].toLowerCase();
        }
    ...
    for (let i = 0; i < a.length; i++) {
      const occToLowerCase = occurancesB[a[i].toLowerCase()];
  
      if (occToLowerCase) {
        if (a[i].charCodeAt() >= 97 && occToLowerCase % 2 === 0) {
          resB += a[i].toLowerCase();
        } else if (a[i].charCodeAt() <= 90 && occToLowerCase % 2 === 0) {
          debugger;
          resB += a[i].toUpperCase();
        } else if (a[i].charCodeAt() >= 97 && occToLowerCase % 2 !== 0) {
          resB += a[i].toUpperCase();
        } else if (a[i].charCodeAt() <= 90 && occToLowerCase % 2 !== 0) {
          resB += a[i].toLowerCase();
        }
