...
function merge(line) {
  let withoutZeros = line.filter(Boolean);
  let merged = [];
  let previousTile = withoutZeros[0];

  for (let i = 1; i < withoutZeros.length; i++) {
    let currentTile = withoutZeros[i];
    if (previousTile === currentTile) {
      merged.push(previousTile + currentTile);
      previousTile = withoutZeros[i + 1];
      i++;
    } else {
      merged.push(previousTile);
      previousTile = currentTile;
    }

    if (i + 1 === withoutZeros.length && previousTile) {
      merged.push(previousTile);
    }
  }
  ...
  return [
    ...merged,
    ...Array.from({ length: line.length - merged.length }, () => 0),
  ];
}
