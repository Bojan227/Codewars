...
function number(busStops){
    // first we loop over the array of the arrays
    let result = 0
    busStops.forEach(([busIn, busOut])=>{
      // then we calculate how many people gets in and out after each bus station
      result += busIn - busOut
    })
    // at the the end we return the number of people in the bus
    return result
    }