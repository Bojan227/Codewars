...
*/
function calculateUsingTheElevator(n, m, speeds) {
  const [a, openDoor, closeDoor, _] = speeds;
  const elevatorToCome = Math.abs(n - m) * a;
  return elevatorToCome + openDoor + closeDoor + n * a + openDoor;
}

function calculateByWalking(n, speeds) {
  return speeds[3] * n;
}

function calculateCombo(n, m, speeds) {
  const [a, openDoor, closeDoor, walk] = speeds;
  const walkToElevator = Math.abs(n - m) * walk;
  const elevator = m * a;
  return walkToElevator + openDoor + closeDoor + elevator + openDoor;
}
...
function shorterestTime(n, m, speeds) {
  ...
  return Math.min(
    ...[
      calculateByWalking(n, speeds),
      calculateCombo(n, m, speeds),
      calculateUsingTheElevator(n, m, speeds),
    ]
  );
}
