...
// followed by an arbitrary number of numeric arrays. Each numeric array contains two numbers, which are indices 
// for elements in arr (the numbers will always be within bounds). For every such array, swap the elements. 
...
const shuffleIt = (arr,...num) => {
   num.forEach(([i, j]) => [arr[j], arr[i]] = [arr[i], arr[j]])
    return arr
  }