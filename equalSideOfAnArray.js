...
the integers to the left of N is equal to the sum of the integers to
the right of N. If there is no index that would make this happen, return -1.
...
because at the 1st position of the array, the sum of left side
of the index ({1}) and the sum of the right side of the index 
...
of N is equal to the side to the right of N.
...
while(counter < arr.length){
  ...
  let left = arr.slice(0, counter)
  let right = arr.slice(counter+1)

  let sum1 = left.reduce((a, b)=>a + b, 0)
  let sum2 = right.reduce((a, b)=>a + b, 0)
