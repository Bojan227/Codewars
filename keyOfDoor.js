...
function findKey(nums) {
    ...
    while (numberOfColumn !== String(nums[0]).length) {
      let tempCache = {};
  
      for (let i = 0; i < nums.length; i++) {
        const currentKey = [String(nums[i])[numberOfColumn]];
  
        if (!tempCache[currentKey]) {
          tempCache[currentKey] = 0;
        }
        tempCache[currentKey] = tempCache[currentKey] += 1;
      }
  
      for (let keys in tempCache) {
        if (tempCache[keys] === 1) {
          key += keys;
        }
      }
