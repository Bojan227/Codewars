...
The parameter runs will always be a positive integer greater than 0 and 
...
function foldArray(array, runs){
    ...
    let mid = length / 2
      ...
      if(runs === 0){
        ...
        if(length % 2 === 0){
          let tempArr = array.slice(0, mid)
          let nextArr = array.slice(mid)
          
          let newArr = []
          let count = 0
          console.log(tempArr.length)
          for(let i = tempArr.length - 1; i >= 0; i--){
            newArr.push(tempArr[count] + nextArr[i])
            count++
          }
          ...
          return foldArray([...newArr, array[Math.floor(mid)]], runs -1)
        }
      }
