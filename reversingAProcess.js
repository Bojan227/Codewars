...
function decode(r) {
  ...
  for (let i = 0; i < r.length; i++) {
    if (!isNaN(parseInt(r.charAt(i)))) {
      getNumber += r.charAt(i);
    } else {
      index = i;
      break;
    }
  }
  let stringToDecode = r.slice(index);
  [...stringToDecode].forEach((char) => {
    const _index = alphabet.indexOf(char);
    for (let i = 0; i < 26; i++) {
      if ((i * getNumber) % 26 === _index) {
        result += alphabet[i];
      }
    }
  });

  return stringToDecode.length === result.length
    ? result
