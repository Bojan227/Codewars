...
Assumption: Assume that Desired Principal 'D' is always greater than the initial principal.
 However it is best to take into consideration that if Desired Principal 
...
function calculateYears(principal, interest, tax, desired) {
   ...
   while(princ < desired){
     let sumInterest = Math.floor(princ * interest)
     princ = princ + sumInterest - Math.floor(sumInterest*tax)
     years+=1 
   }
   return years
