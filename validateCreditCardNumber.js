...
if there are an odd number of digits, double every other digit starting 
with the second:
...
function validate(n){
    let toArr = String(n).split('')
    let length = toArr.length
    console.log(length)
  
    if(length % 2 === 0){
  
      const mapped = toArr.map((num, i)=>{
        if(i%2 ===0){
          let doubled = num*2
  
            if(doubled > 9){
              return String(doubled).split('').reduce((a, b)=> a + parseInt(b), 0)
            }else{
              return doubled
            }
        }else{
      ...
      return mapped.reduce((a,b)=> a + parseInt(b), 0) % 2 === 0 ? true : false
